...
def render(works):
    ...
    for i, (plan, days, label, shows, line, chapter) in enumerate(LEGS):
        ...
        legs.append(
            ...
            f'  <figure><img src="{w["default"]}" srcset="{w["srcset"]}" sizes="(max-width: 760px) 78vw, min(40vw, 38svh, 560px)" '
            f'width="{w["w"]}" height="{w["h"]}" style="--ar:{w["w"] / w["h"]:.4f}" '
            f'alt="{html.escape(f"Engraving by de Neuville or Benett: {shows}", quote=True)}" '
