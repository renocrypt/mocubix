...
- `assets/` — curated material. Only URLs the Wikimedia API actually returned; widths are the buckets it really serves. Never hand-typed, never width-rewritten. `build/curate.py` holds the works list (one `kind` per exhibit: `muybridge`, `iss`, `storm`, `blossfeldt`, `solari`, `planet`, `kilauea`; the Lexicon's `sekka` and `atkins`; and one per showcase: `webb`, `haeckel`, `glacier`, `buson`, `adams`, `fsa`, `gottlieb`, `tabula`, `tarot`, `morris`, `monet`, `posters`, `heures`, `dubois`, `blake`; `python3 build/curate.py <kind>` fetches just one) and merges into `assets/curated.json`. Pages inline the resulting `<img>` tags; nothing is fetched at runtime but the images themselves.
