...
- **DOM owns meaning; the canvas is augmentation.** Every word that matters ships as real DOM text. The canvas is never the only source of any content, heading, link, or LCP candidate.
...
- `artifacts/` — the exhibits. Standalone HTML, opened directly in Chrome. `index.html` is the front door; `lexicon.html` is the Lexicon, the collection's reference wing: every effect named, glossed and running. `house.css` and `house.js` are the two shared files: tokens for both palettes, and the switch.
- `artifacts/parts/` — reusable parts, one custom element per file, each documented in its own header. `split-flap.js` (05) is the first.
- `build/` — the Lexicon generator. `python3 build/build-lexicon.py` rewrites `artifacts/lexicon.html` from `build/lexicon.template.html` plus the specs table. Edit the specs, never the generated file.
...
- `notes/evidence/` — before/after screenshots and measurement crops per exhibit, named `NN-before-*`, `NN-after-*`.
