...
- `build/` — `python3 build/build-lexicon.py` writes the Lexicon's pages (index, pillars, terms) and `site/lexicon/material.js` from the specs table; the demos' styles and script are hand-edited in `site/lexicon/lexicon.css` and `lexicon.js`. Edit the specs, never a generated page. A term becomes a showcase when `build/showcases/<term>.py` exists (TITLE, LEDE, HOW, NOTE, `render(works)`, optionally GLOSS, MATERIAL, HEAD for extra head markup such as a font, `credit(works)`), with `showcase.css` and, if it must, a small `showcase.js` beside its page. The old demos' styles live inside `@scope (.spec)` in `lexicon.css`, so a showcase may use any class name without inheriting a card's rules. `python3 build/register.py <showcase>` lays each later photograph of a pair over the earlier one (patches on stable rock matched and agreed on, a least-squares fit) into `assets/registration.json`; `python3 build/register.py stitch <showcase>` lays a showcase's overlapping SHEETS end to end into one strip (a map's band found between its blank margins, or BAND as fractions for a textile; sheets of different scale brought to one height before matching), each placed by a shift, a turn, a scale and a cut. `python3 build/palette.py <showcase>` takes a ground, ink and accent from each of a showcase's PAINTINGS (OKLab, out of the unreadable middle band) into `assets/palettes.json`. `python3 build/cutout.py <kind>` cuts every work of a kind out of the backing it was scanned on (a sleeve, a scanner bed: light, colourless, reaching the edge) and writes its outline as a clip-path polygon into `assets/cutouts.json`, so a torn board floats on any ground. `python3 build/sitemap.py` rewrites the sitemap from the pages that exist. `python3 build/serve.py` previews the site.
- `assets/` — curated material. Only URLs the Wikimedia API actually returned; widths are the buckets it really serves. Never hand-typed, never width-rewritten. `build/curate.py` holds the works list (one `kind` per exhibit: `muybridge`, `iss`, `storm`, `blossfeldt`, `solari`, `planet`, `kilauea`; the Lexicon's `sekka` and `atkins`; and one per showcase: `webb`, `haeckel`, `glacier`, `buson`, `adams`, `fsa`, `gottlieb`, `tabula`, `tarot`, `morris`, `monet`, `posters`, `heures`, `dubois`, `blake`, `prokudin`, `bayeux`, `caslon`, `verne`; `python3 build/curate.py <kind>` fetches just one) and merges into `assets/curated.json`. Pages inline the resulting `<img>` tags; nothing is fetched at runtime but the images themselves.
